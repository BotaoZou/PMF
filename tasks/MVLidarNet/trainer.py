...
class Trainer(object):
    ...
    def _initDataloader(self):
        ...
        train_pv_loader = pc_processor.dataset.SalsaNextLoader(
            dataset=trainset,
            config=self.settings.config,
            is_train=True)

        val_pv_loader = pc_processor.dataset.SalsaNextLoader(
            dataset=valset,
            config=self.settings.config,
            is_train=False)
    ...
    def run(self, epoch, mode="Train"):
        ...
        for i, (input_feature, input_label, input_mask) in enumerate(dataloader):
            ...
            if mode == "Train":
                ...
                total_loss = loss_cross_entropy(lidar_pred, input_label)
                if self.settings.n_gpus > 1:
