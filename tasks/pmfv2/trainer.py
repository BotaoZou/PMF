...
class Trainer(object):
    ...
    def _initOptimizer(self):
        ...
        if self.settings.use_downscale:
            adam_params.append({"params": self.model.downconv_lidar.parameters()})
            sgd_params.append({"params": self.model.downconv_cam.parameters()})
    ...
    def run(self, epoch, mode="Train"):
        ...
        for i, input_data in enumerate(dataloader):
            ...
            B,N,C,H,W = input_data.size()
            input_feature = input_data.view(B*N, C, H, W).contiguous()
            # input_feature = input_data[:, 0].cuda() # keep only FV
            ...
            rgb_label = input_label.clone() * rgb_mask
            ...
            if mode == "Train":
                ...
                loss_lov_cam, loss_foc_cam = self._computeClassifyLoss(
                    pred=camera_pred, label=rgb_label, label_mask=label_mask*rgb_mask)
            ...
            with torch.no_grad():
                ...
                self.metrics_img.addBatch(argmax_img, rgb_label)
